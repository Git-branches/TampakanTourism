...
/* The one page open to an officer still on a password somebody else set:
   it is where they replace it. */
Auth::require(passwordPage: true);
...
if (is_post()) {
    ...
    if ($action === 'password') {
        ...
        /* This browser continues on the new password; every other one that was
           signed in to this account is signed out on its next click. */
...
<?php if (Auth::mustChangePassword()): ?>
    <div class="alert alert-warning" role="alert">
        ...
        The one you signed in with was set by another officer, so someone else knows it.
        Enter it as your current password below and choose a new one &mdash; the rest of the
        system opens as soon as you do.
    </div>
<?php elseif ($neverChanged): ?>
...
<div class="panel-row">
    <div>
        <section class="panel" id="password">
            <?php section_head('fa-key', 'Change Password', 'Your own sign-in password. Changing it signs out every other device using this account.') ?>
